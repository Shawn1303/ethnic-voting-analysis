			...
			<Collapse in={open} timeout='auto' unmountOnExit style={{maxHeight: '75vh', overflowY: 'auto'}}>
				{
					props.state === ''? (
						...
						<Grid container spacing={2} >
                            <Grid item xs={4}> 
                                <b>State Population:</b> {Intl.NumberFormat('en-US', { style: 'decimal' }).format(stateSummary.data[0][0].registered_voters_total)}
                            </Grid>
                            <Grid item xs={12}>
                                <b>Population of Each Significant Racial/Ethnic Group:</b>
								<RacialDistributionBar stateSummary={stateSummary.data}/>
                            </Grid>
							<Grid item xs={4}>
								<RacialDistributionTable stateSummary={stateSummary.data}/>
                            </Grid>
							<Grid item xs={8}>
                                <b>State Voter Distribution:</b>
								<PartyDistributionBar stateSummary={stateSummary.data}/>
                            </Grid>
						</Grid>
					)
				}
